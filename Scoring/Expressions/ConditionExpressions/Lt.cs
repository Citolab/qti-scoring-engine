...
{
    internal class Lt : ConditionExpressionBase
    {
        public override bool Execute(IProcessingContext ctx)
        {

            var values = expressions.Select(e => e.Apply(ctx)).ToList();
            ctx.LogInformation($"lt check. Values: {string.Join(", ", values.Select(v => v.Value).ToArray())}");
            if (values.Count != 2)
            {
                ...
                case BaseType.Float:
                    {
                        ...
                        {
                            ctx.LogError($"value cannot be casted to numeric value in lt operator: {values[0]?.Value}, {values[1]?.Value}");
                        }
                        break;
                    }
                default:
                    {
                        ctx.LogError($"values other than float and int cannot be used in lt operator.");
                        break;
                    }
            }
            return false;
