...
{
    internal class Lte : IConditionExpression
    {
        public string Name => "qti-lte";
        ...
        public bool Execute(XElement qtiElement, IProcessingContext context)
        {
            ...
            context.LogInformation($"lte check. Values: {string.Join(", ", values.Select(v => v.Value).ToArray())}");
            if (values.Count() != 2)
            {
                ...
                case BaseType.Float:
                    {
                        ...
                        {
                            context.LogError($"value cannot be casted to numeric value in lte operator: {values[0]?.Value}, {values[1]?.Value}");
                        }
                        break;
                    }
                default:
                    {
                        context.LogError($"values other than float and int cannot be used in lte operator.");
                        break;
                    }
            }
            return false;
