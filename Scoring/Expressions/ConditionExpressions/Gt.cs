...
{
    internal class Gt : ConditionExpressionBase
    {
        public override bool Execute(IProcessingContext ctx)
        {

            var values = expressions.Select(e => e.Apply(ctx)).ToList();
            ctx.LogInformation($"gt check. Values: {string.Join(", ", values.Select(v => v.Value).ToArray())}");
            if (values.Count != 2)
            {
                ...
                case BaseType.Float:
                    {
                        ...
                        {
                            ctx.LogError($"value cannot be casted to numeric value in gt operator: {values[0]?.Value}, {values[1]?.Value}");
                        }
                        break;
                    }
                default:
                    {
                        ctx.LogError($"values other than float and int cannot be used in gt operator.");
                        break;
                    }
            }
            return false;
